...
class ASTGeneration(CSlangVisitor):
    ...
    def visitDecllist(self, ctx:CSlangParser.DecllistContext):
        if ctx.getChildCount() == 0:
            return []
        return [self.visit(ctx.classdecl())] + self.visit(ctx.declist())

    def visitClassdecl(self, ctx:CSlangParser.ClassdeclContext):
        if ctx.superpart() is None:
            return ClassDecl(ctx.ID().getText(), self.visit(ctx.memberlist()))
        return ClassDecl(ctx.ID().getText(), ctx.ID().getText(), self.visit(ctx.memberlist()))
    ...
    def visitIdentifer(self, ctx:CSlangParser.IdentifierContext):
        return ID() if ctx.ID() else ATIDENTIFIER()
